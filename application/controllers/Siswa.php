...
class Siswa extends CI_Controller
{
	...
	public function edit($id)
	{
		...
		$data = [
			...
			'kelas'		=> $this->master->getKelasByJurusan($sws->jurusan_id),
			'siswa' => $sws
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	public function validasi_siswa($method)
	{
		...
		if ($method == 'add') {
			$u_nama = '|is_unique[siswa.nama]';
			$u_email = '|is_unique[siswa.email]';
		} else {
			$dbdata 	= $this->master->getSiswaById($id_siswa);
			$u_nama		= $dbdata->nama === $nama ? "" : "|is_unique[siswa.nama]";
